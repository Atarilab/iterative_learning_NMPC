...
class ReferenceVisualCallback(VisualCallback):
    ...
    def add_visuals(self, mj_data):
        ...
        if self.simulator and self.simulator.visualize_force and self.simulator.apply_force:
            for start_step, end_step, vec in self.simulator.force_schedules:
                if start_step <= self.simulator.sim_step < end_step:
                    body_id = mujoco.mj_name2id(self.simulator.mj_model, mujoco.mjtObj.mjOBJ_BODY, self.simulator.force_body_name)
                    if body_id < 0:
                        continue
                    fx = vec[:3]
                    p0 = mj_data.xpos[body_id]
...
class StateDataRecorder(DataRecorder):
    def __init__(
        ...
        # init time stamp
        self.timestamp = self.get_date_time_str()
    ...
    def save(self) -> None:
        ...
        if self.nominal_flag:
            file_path = os.path.join(self.record_dir, f"traj_nominal_{self.timestamp}.npz")
        else:
            # file_path = os.path.join(self.record_dir, f"traj_{self.replanning_point}_{self.nth_traj_per_replanning_point}.npz")
            file_path = os.path.join(self.record_dir, f"traj_{self.timestamp}.npz")
        try:
...
def rollout_mpc_phase_percentage_shift(robot_name = "go2",
                ...
                force_start_times: List[float] = None,
                force_durations: List[float] = None,
                force_vecs: List[np.ndarray] = None,
                ):
    ...
    if force_start_times is not None and force_vecs is not None:
        for start_time, duration, vec in zip(force_start_times, force_durations, force_vecs):
            start_step = int(start_time / sim.sim_dt)
            end_step = int((start_time + duration) / sim.sim_dt)
            sim.force_schedules.append((start_step, end_step, vec))
        sim.apply_force = True
    print("force schedules are = ", sim.force_schedules)
    # input()
