## NOTE: This script is for data collection for multiple velocity conditioned goals for behavior cloning.
import sys
import os
...
class DataCollection():
    def __init__(self, cfg, iteration:int):
        ...
        self.data_save_path = self._prepare_save_path(iteration)
        
    
    def _prepare_save_path(self, iteration: int):
        current_time = datetime.now().strftime("%b_%d_%Y_%H_%M_%S")
        base_path = os.path.join(
            self.cfg.data_save_path,
            "behavior_cloning",
            '_'.join(self.gaits)
        )
        if self.cfg.suffix:
        ...
        return os.path.join(base_path, f"iter_{iteration}_{current_time}", "dataset")
